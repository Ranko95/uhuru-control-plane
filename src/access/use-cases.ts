...
function status(value: repository.AccessState) {
  return value.revoked_at ? 'revoked' : value.within_term ? 'active' : 'expired';
}
...
export async function listUsers(db: Database) { return repository.readUsers(db); }
export async function getPlan(db: Database) { return repository.readPlan(db); }
...
async function profileLink(db: PoolClient, profileId: string, origin: string) {
  ...
  return { ...visible, status: commercialStatus,
    ...(commercialStatus === 'revoked' ? {} : { link: origin + '/s/' + link_secret.toString('base64url') }) };
}

export async function issueFirstProfile(pool: Pool, userId: string, origin: string) {
...
export async function showProfileLink(pool: Pool, profileId: string, origin: string) {
...
export async function listProfiles(db: Database, userId: string) {
  return (await repository.readProfiles(db, userId)).map(p => ({
    id: p.id, revoked_at: p.revoked_at, status: status(p),
  }));
}
...
export async function authorizeSubscriptionLink(db: PoolClient, linkSecret: Buffer) {
  const p = await repository.readProfileBySecret(db, linkSecret);
  if (!p) throw new AccessError('not_found');
  const commercialStatus = status(p);
  if (commercialStatus === 'revoked' || commercialStatus === 'expired') throw new AccessError(commercialStatus);
