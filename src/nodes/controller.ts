...
export const nodeAdminController: FastifyPluginAsync<{ pool: Pool }> = async (app, { pool }) => {
  app.post<{ Body: { label: string; public_connection: Connection; bearer: string } }>('/nodes', {
    ...
    return reply.code(201).send(await nodes.registerNode(pool, req.body.label.trim(), req.body.public_connection, raw));
  });
  app.put<{ Params: { id: string }; Body: { bearer: string } }>('/nodes/:id/bearer', {
  ...
  app.get<{ Params: { id: string } }>('/profiles/:id/readiness', { schema: { params: idParams } },
    async req => nodes.getProfileReadiness(pool, req.params.id));
};

export const nodeAgentController: FastifyPluginAsync<{ pool: Pool }> = async (app, { pool }) => {
  app.post<{ Body: Report }>('/sync', { schema: { body: reportSchema } }, async req => {
