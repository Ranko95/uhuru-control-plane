...
export async function writeDesiredSnapshot(
  db: PoolClient,
  nodeId: string,
...
type ReadinessNode = PublicNode &
  Diagnostics & { confirmed_snapshot: StoredSnapshot | null };
...
export async function replaceSecretHash(
  db: Database,
  nodeId: string,
  hash: Buffer,
) {
  return !!(
    await db.query('UPDATE nodes SET agent_secret_hash=$2 WHERE id=$1', [
      nodeId,
      hash,
    ])
  ).rowCount;
...
export async function readIncludedNodes(db: PoolClient) {
  return (
    await db.query<
      Pick<
        ReadinessNode,
        'id' | 'label' | 'public_connection' | 'confirmed_snapshot'
      >
    >(
      'SELECT n.id,n.label,n.public_connection,s.confirmed_snapshot FROM nodes n ' +
        'JOIN node_sync s ON s.node_id=n.id WHERE n.include_in_subscription ORDER BY n.id',
    )
  ).rows;
}
export async function lockNodeBySecretHash(db: PoolClient, hash: Buffer) {
  return (
    await db.query<{
      ...
      'SELECT id,agent_secret_hash,public_connection FROM nodes WHERE agent_secret_hash=$1 FOR UPDATE',
      [hash],
    )
  ).rows[0];
}
export async function lockSyncState(db: PoolClient, nodeId: string) {
  return (
    await db.query<SyncState>(
      'SELECT desired_snapshot,sent_snapshot,confirmed_snapshot FROM node_sync WHERE node_id=$1 FOR UPDATE',
      [nodeId],
    )
  ).rows[0];
}
export async function writeSyncResult(
  ...
  await db.query(
    'UPDATE node_sync SET confirmed_snapshot=$2,' +
