...
export async function rotateBearer(pool: Pool, nodeId: string, bearer: Buffer) {
  if (!(await repository.replaceSecretHash(pool, nodeId, digest(bearer))))
    throw new NodeError('not_found');
}
export async function listNodes(db: Database) {
  return repository.readNodes(db);
}
...
export async function synchronize(pool: Pool, bearer: Buffer, report: Report) {
  return transaction(pool, async (db) => {
    ...
    const sent =
      row.sent_snapshot &&
      checked(row.sent_snapshot, node.id, node.public_connection.inbound_tag);
    let confirmed =
    ...
    const known = [desired, sent, confirmed].filter(
      (s): s is StoredSnapshot => !!s,
    );
    // Never silently repair a restored/corrupt database by lowering any revision.
    for (const a of known) {
      if (BigInt(a.revision) > BigInt(desired.revision))
    ...
    for (const ref of [report.saved, report.verified]) {
      ...
      for (const s of known)
        if (s.revision === ref.revision && !same(s, ref))
          throw new NodeError('state_conflict');
    }
    let ack = 'none';
    if (report.verified) {
      if (same(report.verified, confirmed)) ack = 'already_confirmed';
        ...
        confirmed &&
        BigInt(report.verified.revision) < BigInt(confirmed.revision)
      )
        ack = 'ignored_stale';
      else if (same(report.verified, sent)) {
        ack = 'accepted';
        confirmed = sent;
      } else if (BigInt(report.verified.revision) < BigInt(desired.revision))
        ack = 'unrecognized';
      else throw new NodeError('state_conflict');
    }
    const upToDate =
      same(report.verified, desired) &&
      ['accepted', 'already_confirmed'].includes(ack);
