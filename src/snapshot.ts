...
export function snapshot(
  ...
  if (!/^[1-9][0-9]{0,18}$/.test(revision) || BigInt(revision) > maxRevision)
    throw new Error('invalid_revision');
  if (
    !new RegExp(uuidPattern).test(nodeId) ||
    !/^[a-zA-Z0-9_-]{1,64}$/.test(inbound) ||
  ...
  const users = profiles
    .map((p) => {
      if (
        !new RegExp(uuidPattern).test(p.profile_id) ||
        !new RegExp(uuidPattern).test(p.vless_uuid) ||
        Object.keys(p).sort().join() !== 'profile_id,vless_uuid' ||
        ids.has(p.profile_id) ||
        credentials.has(p.vless_uuid)
      ) {
        throw new Error('invalid_profiles');
      }
      ids.add(p.profile_id);
      credentials.add(p.vless_uuid);
...
export function checked(
  ...
  if (
    Object.keys(value).sort().join() !== 'revision,snapshot,snapshot_hash' ||
    Object.keys(value.snapshot).sort().join() !==
      'inbound_tag,node_id,revision,users' ||
    value.snapshot.node_id !== nodeId ||
    value.snapshot.inbound_tag !== inbound ||
  ...
  if (canonical.snapshot_hash !== value.snapshot_hash)
    throw new Error('corrupt_snapshot');
  return canonical;
}
