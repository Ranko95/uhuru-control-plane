...
async function request(
  ...
  path: string,
  payload?: unknown,
  auth: string | null = authorization,
) {
  return new Promise<{
    status: number;
...
before(async () => {
  ...
  const grants = await readFile(
    new URL('../deploy/app-role.sql', import.meta.url),
    'utf8',
  );
...
beforeEach(async () => {
  await db.query(
    'TRUNCATE users, access_profiles, subscriptions, nodes, node_sync',
  );
});
after(async () => {
  await app?.close();
...
test('commercial use cases own issuance and their controllers retain validation and authentication', async () => {
  ...
  assert.equal(
    (await request('POST', '/admin/users', { label: '   ' })).status,
    400,
  );
  assert.equal(
    (await request('POST', '/admin/users/not-a-uuid/first-profile')).status,
    400,
  );
  const first = await access.issueFirstProfile(
    pool,
    user.id,
    'https://localhost',
  );
  assert.deepEqual(
    await access.showProfileLink(pool, first.profile_id, 'https://localhost'),
    first,
  );
  assert.deepEqual(
    await access.issueFirstProfile(pool, user.id, 'https://localhost'),
    first,
  );
  assert.equal(
    first.ends_at.getTime() - first.starts_at.getTime(),
    2_592_000_000,
  );
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof access.AccessError && error.reason === 'not_found',
  );
});
...
test('a subscription requires a first profile owned by the same user', async () => {
  const owner = randomUUID(),
    other = randomUUID(),
    profile = randomUUID();
  await db.query('INSERT INTO users(id,label) VALUES ($1,$3),($2,$3)', [
    owner,
    other,
    'test user',
  ]);
...
test('HTTPS admin creates a user and safely repeats first issuance and link display', async () => {
  ...
  const user = (
    await request('POST', '/admin/users', { label: 'Alice' })
  ).json();
  ...
  assert.equal(
    new Date(first.ends_at).getTime() - new Date(first.starts_at).getTime(),
    2_592_000_000,
  );
  assert.equal(new URL(first.link).protocol, 'https:');
  const repeated = (
    await request('POST', `/admin/users/${user.id}/first-profile`)
  ).json();
  assert.ok(
  ...
  const shown = await request(
    'POST',
    `/admin/profiles/${first.first_profile_id}/link`,
  );
  assert.ok(shown.json().link === first.link);
  const list = (
    await request('GET', `/admin/users/${user.id}/profiles`)
  ).json();
...
async function issue() {
  const user = (
    await request('POST', '/admin/users', { label: 'Subscriber' })
  ).json();
...
async function sync(
  ...
  return request(
    'POST',
    '/agent/v1/sync',
    { node_id: node.id, saved, verified, error },
    `Bearer ${node.bearer}`,
  );
}
test('only a sent and verified snapshot unlocks the profile, with stable first confirmation', async () => {
  ...
  assert.equal(
    (await request('GET', profile.path, undefined, null)).status,
    503,
  );
  ...
  const ready = (
    await request(
      'GET',
      `/admin/profiles/${profile.first_profile_id}/readiness`,
    )
  ).json();
  ...
  assert.equal(
    (await sync(node, pending.desired, pending.desired)).json().ack_status,
    'already_confirmed',
  );
  const repeated = (
    await request(
...
test('configuration delivery works without HTTP and preserves URI encoding and response headers', async () => {
  ...
  assert.ok(
    response.text === body,
    'controller must return the use-case result unchanged',
  );
  ...
  assert.ok(
    uri.username === pending.snapshot!.users[0].vless_uuid,
    'URI must contain the confirmed credential',
  );
  ...
  const escapedPath =
    '/s/%' + p.path.charCodeAt(3).toString(16) + p.path.slice(4);
  assert.equal(
    (await request('GET', escapedPath, undefined, null)).status,
    404,
  );
});
...
test('parallel first issuance and server restart preserve one first profile and term', async () => {
  const node = await registerNode();
  const user = (
    await request('POST', '/admin/users', { label: 'Concurrent' })
  ).json();
  const results = await Promise.all(
    Array.from({ length: 8 }, () =>
      request('POST', `/admin/users/${user.id}/first-profile`),
    ),
  );
  assert.ok(
    results.every((r) => r.status === 200 && r.text === results[0].text),
  );
  ...
  const afterRestart = await request(
    'POST',
    `/admin/users/${user.id}/first-profile`,
  );
  assert.ok(afterRestart.text === results[0].text);
  assert.equal(
    (await request('GET', `/admin/users/${user.id}/profiles`)).json().length,
    1,
  );
  assert.equal((await sync(node)).json().desired.revision, '2');
});
...
test('a lost first issuance response is recovered after restart with the same profile and link', async () => {
  const user = (
    await request('POST', '/admin/users', { label: 'Lost response' })
  ).json();
...
test('failure before commit rolls back the profile, subscription and all desired changes', async () => {
  const a = await registerNode(),
    b = await registerNode();
  const before = [
    (await sync(a)).json().desired,
    (await sync(b)).json().desired,
  ];
  const user = (
    await request('POST', '/admin/users', { label: 'Rollback' })
  ).json();
  ...
  try {
    assert.equal(
      (await request('POST', `/admin/users/${user.id}/first-profile`)).status,
      503,
    );
  } finally {
    await db.query(
      'DROP TRIGGER fail_issuance ON subscriptions; DROP FUNCTION fail_issuance()',
    );
  }
  assert.equal(
    (await request('GET', `/admin/users/${user.id}/profiles`)).json().length,
  ...
  assert.deepEqual(
    [(await sync(a)).json().desired, (await sync(b)).json().desired],
    before,
  );
  assert.equal(
    (await request('POST', `/admin/users/${user.id}/first-profile`)).status,
...
test('issuance takes PostgreSQL time after waiting for both user and node locks', async () => {
  ...
  for (const table of ['users', 'nodes']) {
    const user = (
      await request('POST', '/admin/users', { label: 'Lock wait' })
    ).json();
    ...
    try {
      ...
      while (
        ...
        assert.ok(
          Date.now() < deadline,
          'HTTP issuance must reach the held lock',
        );
        await new Promise((r) => setTimeout(r, 10));
      }
      const releasedAt = (await blocker.query('SELECT clock_timestamp() AS t'))
        .rows[0].t;
      await blocker.query('COMMIT');
      const result = await issuing;
      assert.equal(result.status, 200);
      assert.ok(
        new Date(result.json().starts_at).getTime() >= releasedAt.getTime(),
      );
    } finally {
...
test('database constraints and UTC hours preserve secrets, ownership and exact expiry boundaries', async () => {
  ...
  const {
    rows: [raw],
  } = await db.query('SELECT * FROM access_profiles WHERE id=$1', [
    p.first_profile_id,
  ]);
  ...
  for (const values of [
    ...
    await assert.rejects(
      db.query(
        'INSERT INTO access_profiles(id,user_id,vless_uuid,link_secret) VALUES ($1,$2,$3,$4)',
        [values[0], p.user.id, values[1], values[2]],
      ),
      { code: '23505' },
    );
  }
  await assert.rejects(
    db.query("UPDATE subscriptions SET ends_at='infinity' WHERE user_id=$1", [
      p.user.id,
    ]),
    { code: '23514' },
  );
  await assert.rejects(
    db.query('UPDATE subscriptions SET ends_at=starts_at WHERE user_id=$1', [
      p.user.id,
    ]),
    { code: '23514' },
  );
  await assert.rejects(
    pool.query('UPDATE access_profiles SET vless_uuid=$2 WHERE id=$1', [
      raw.id,
      randomUUID(),
    ]),
    { code: '42501' },
  ...
  await assert.rejects(
    pool.query(
      'UPDATE subscriptions SET first_profile_id=$2 WHERE user_id=$1',
      [p.user.id, randomUUID()],
    ),
    { code: '42501' },
  );
  await assert.rejects(pool.query('DELETE FROM users'), { code: '42501' });
  assert.equal(
    (await request('GET', '/admin/plan')).json().price_kopecks,
    null,
  );
  for (const start of [
    ...
    const {
      ...
      FROM subscriptions s WHERE user_id=$1`,
      [p.user.id],
    );
...
test('expired and revoked prepared profiles are never renewed or replaced by issuance or display', async () => {
  const p = await issue();
  await db.query(
    "UPDATE subscriptions SET starts_at=clock_timestamp()-interval '721 hours', ends_at=clock_timestamp()-interval '1 hour' WHERE user_id=$1",
    [p.user.id],
  );
  const expired = (
    await request('POST', `/admin/users/${p.user.id}/first-profile`)
  ).json();
  assert.equal(expired.status, 'expired');
  assert.ok(expired.link === p.link);
  assert.equal((await request('GET', p.path, undefined, null)).status, 403);
  await db.query(
    'UPDATE access_profiles SET revoked_at=clock_timestamp() WHERE id=$1',
    [p.first_profile_id],
  );
  for (const path of [
    `/admin/users/${p.user.id}/first-profile`,
    `/admin/profiles/${p.first_profile_id}/link`,
  ]) {
...
test('Node use cases roll back a failed ACK commit and keep readiness separate from inclusion and access', async () => {
  ...
  const before = (
    await db.query('SELECT * FROM node_sync WHERE node_id=$1', [node.id])
  ).rows[0];
  ...
  try {
    ...
    await db.query(
      'DROP TRIGGER fail_ack ON node_sync; DROP FUNCTION fail_ack()',
    );
  }
  const after = (
    await db.query('SELECT * FROM node_sync WHERE node_id=$1', [node.id])
  ...
  assert.ok(
    ...
    'failed commit must preserve all sync state',
  );
  assert.equal(
    (await nodes.synchronize(pool, bearer, ack)).ack_status,
    'accepted',
  );
  ...
  await assert.rejects(
    ...
    (error) =>
      error instanceof nodes.NodeError && error.reason === 'unauthorized',
  );
  assert.equal(
    (await nodes.synchronize(pool, replacement, ack)).ack_status,
    'already_confirmed',
  );
  await db.query('UPDATE nodes SET include_in_subscription=false WHERE id=$1', [
    node.id,
  ]);
  ...
  const historical = (
    await nodes.getProfileReadiness(pool, p.first_profile_id)
  )[0];
...
test('node Bearer binds one node and rotates without overlap or resetting access', async () => {
  ...
  const hash = (
    await db.query('SELECT agent_secret_hash FROM nodes WHERE id=$1', [node.id])
  ).rows[0].agent_secret_hash;
  assert.equal(
    (await sync({ ...node, bearer: hash.toString('base64url') })).status,
    401,
  );
  assert.equal(
    (
  ...
  assert.ok(
    !nodes.includes(replacement) &&
      !nodes.includes(initial.snapshot.users[0].vless_uuid),
  );
});
...
test('canonical inputs and duplicate keys are rejected without revealing secrets', async () => {
  ...
  for (const suffix of ['=', '?x=1', '/']) {
    assert.equal(
      (await request('GET', p.path + suffix, undefined, null)).status,
      404,
    );
  }
  ...
  assert.equal(
    (await request('GET', `/s/${alias}`, undefined, null)).status,
    404,
  );
  assert.equal(
    (
  ...
  assert.equal(
    (await request('POST', '/agent/v1/sync', body, `Bearer ${node.bearer}`))
      .status,
    400,
  );
  const escaped = `{"node_id":"${node.id}","saved":null,"verified":null,"error":{"target":null,"stage":"apply","code":"failed","co\\u0064e":"failed"}}`;
  assert.equal(
    (await request('POST', '/agent/v1/sync', escaped, `Bearer ${node.bearer}`))
      .status,
    400,
  );
  assert.equal(
...
test('ACK provenance, all statuses and late diagnostics preserve monotone confirmation', async () => {
  const node = await registerNode();
  const stored = (
    await db.query('SELECT desired_snapshot FROM node_sync WHERE node_id=$1', [
      node.id,
    ])
  ).rows[0].desired_snapshot;
  const initial = {
    revision: stored.revision,
    snapshot_hash: stored.snapshot_hash,
  };
  assert.equal(
    (await sync(node, initial, initial)).status,
    409,
    'current desired was not sent yet',
  );
  ...
  const forgotten = (
    await sync(node, acceptedOld.desired, acceptedOld.desired)
  ).json();
  ...
  assert.equal(
    (await sync(node, acceptedOld.desired, acceptedOld.desired)).json()
      .ack_status,
    'ignored_stale',
  );
  const diagnostic = {
  ...
  assert.equal(
    (await sync(node, null, null, diagnostic)).json().ack_status,
    'none',
  );
  const after = (
    await request('GET', `/admin/profiles/${p.first_profile_id}/readiness`)
  ...
  assert.equal(
    (await sync(node, latest.desired, latest.desired)).json().ack_status,
    'already_confirmed',
  );
  assert.equal(
    (await sync(node, { ...latest.desired, snapshot_hash: '0'.repeat(64) }))
      .status,
    409,
  );
  assert.equal(
    (await sync(node, { ...latest.desired, revision: '4' })).status,
    409,
  );
  ...
  assert.equal(
    (await sync(node)).status,
    409,
    'restored counters must not be reset automatically',
  );
});
...
test('the real-agent JCS golden snapshot verifies sorted users and fails closed on database corruption', async () => {
  ...
  await db.query(
    ...
    [
      ...
      connection,
      createHash('sha256')
        .update(Buffer.from(node.bearer, 'base64url'))
        .digest(),
    ],
  );
  await db.query(
    'INSERT INTO node_sync(node_id,desired_snapshot) VALUES ($1,$2)',
    [node.id, golden],
  );
  ...
  assert.equal(
    response.json().snapshot.users[0].profile_id,
    value.users[1].profile_id,
  );
  await db.query('UPDATE node_sync SET desired_snapshot=$2 WHERE node_id=$1', [
    node.id,
...
test('prepared profile limits include every unrevoked profile and readiness requires the exact credential', async () => {
  const user = (
    await request('POST', '/admin/users', { label: 'Prepared limit' })
  ).json();
  for (let i = 0; i < 3; i++) {
    await db.query(
      'INSERT INTO access_profiles(id,user_id,vless_uuid,link_secret) VALUES ($1,$2,$3,$4)',
  ...
  await db.query('UPDATE access_profiles SET vless_uuid=$2 WHERE id=$1', [
    p.first_profile_id,
    randomUUID(),
  ]);
  assert.equal((await request('GET', p.path, undefined, null)).status, 503);
